...
public class SecurityConfig {
    ...
    private static final String[] PUBLIC_POST = {
            ...
            // Razorpay webhook is server-to-server; no user JWT, but
            // signature-verified inside PaymentWebhookController.
            "/api/payments/webhook"
    };

    private static final String[] PUBLIC_GET = {
            "/api/products/**", "/api/reviews/**", "/api/public/**", "/api/categories",
            "/swagger-ui/**", "/swagger-ui.html", "/api-docs/**", "/actuator/health",
            // ISSUE-16 FIX: the email-verification endpoint must be publicly
            // accessible because the user has no JWT at the time they click the
            // link — tokens are only issued after a successful login, and login
            // requires a verified account.  Omitting this path forced every
            // unverified user through a 401 Unauthorized wall they could not pass.
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        .requestMatchers(HttpMethod.GET, "/api/reviews/mine/**").hasRole("CUSTOMER")
                        ...
                        .requestMatchers(HttpMethod.POST, "/api/orders/verify-payment").hasRole("CUSTOMER")
                        .requestMatchers("/api/orders/my-orders").hasRole("CUSTOMER")
                        .requestMatchers(HttpMethod.POST, "/api/returns").hasRole("CUSTOMER")
                        .requestMatchers(HttpMethod.POST, "/api/orders/*/cancel").hasRole("CUSTOMER")
                        // Vendor-only
                        .requestMatchers("/api/vendor/**").hasRole("VENDOR")
                        .requestMatchers(HttpMethod.POST, "/api/categories/request").hasRole("VENDOR")
                        ...
                        // Shared
                        .requestMatchers("/api/wallet/**").hasAnyRole("CUSTOMER", "VENDOR")
                        // Authenticated
                        .requestMatchers("/api/notifications/**").authenticated()
                        .requestMatchers(HttpMethod.PUT, "/api/orders/*/status", "/api/orders/*/tracking")
                        .hasRole("VENDOR")
    ...
    public CorsConfigurationSource corsConfigurationSource() {
        ...
        List<String> origins = StringUtils.hasText(allowedOriginsRaw)
